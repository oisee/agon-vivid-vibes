#!/usr/bin/env python3
"""Generate a spinning cube rendered as filled triangles.

Outputs:
  --html FILE    self-contained HTML canvas animation for debugging
  --preview      stream live to agon-vdp-sdl via fake_ez80 TCP server
  --output FILE  save to a .vdp file for later playback

Uses the same 3D engine as gen_torus.py: perspective projection, backface
culling, painter's algorithm, diffuse shading to Agon 64-colour palette.
"""
...
# -- Camera / projection --
CAMERA_Z = -6.0   # camera position (looking toward +Z)
FOV_FACTOR = 250   # perspective scale factor

# -- Lighting --
# Direction TO the light source, in world coords.
# Camera at Z=-6 looking toward +Z. Visible faces have normals toward -Z.
# Light from slightly above, near the camera.
LIGHT_DIR = np.array([0.1, 0.3, -0.9])
LIGHT_DIR /= np.linalg.norm(LIGHT_DIR)
AMBIENT = 0.3

# -- Face colours: distinct base colours for each cube face --
# (r_scale, g_scale, b_scale) multipliers for luminance -> Agon colour
# These give each face a different hue while still responding to lighting.
FACE_COLOURS = [
    (1.0, 0.2, 0.2),   # red-ish      (+Z face, tris 0-1)
    (0.2, 0.2, 1.0),   # blue-ish     (-Z face, tris 2-3)
    (0.2, 1.0, 0.2),   # green-ish    (+X face, tris 4-5)
    (1.0, 1.0, 0.2),   # yellow-ish   (-X face, tris 6-7)
    (1.0, 0.2, 1.0),   # magenta-ish  (+Y face, tris 8-9)
    (0.2, 1.0, 1.0),   # cyan-ish     (-Y face, tris 10-11)
]


def make_cube_mesh() -> tuple[np.ndarray, list[tuple[int, int, int]], list[int]]:
    """Generate cube vertices, triangle indices, and face assignments.

    Returns:
        vertices: (8, 3) array of 3D positions
        triangles: list of 12 (i, j, k) index triples
        face_ids: list of 12 face indices (0-5), one per triangle
    """
    ...
    # Face 0: front  (+Z) — vertices 4,5,6,7
    # Face 1: back   (-Z) — vertices 0,3,2,1
    # Face 2: right  (+X) — vertices 1,2,6,5
    # Face 3: left   (-X) — vertices 0,4,7,3
    # Face 4: top    (+Y) — vertices 3,7,6,2
    # Face 5: bottom (-Y) — vertices 0,1,5,4
    triangles = [
        (4, 5, 6), (4, 6, 7),   # front  +Z
        (0, 3, 2), (0, 2, 1),   # back   -Z
        (1, 2, 6), (1, 6, 5),   # right  +X
    ...
    face_ids = [i // 2 for i in range(12)]

    return vertices, triangles, face_ids
...
def compute_normals(vertices: np.ndarray,
...
def project(point: np.ndarray) -> tuple[int, int]:
    ...
    Agon VDP in mode 8 uses pixel coordinates: origin at top-left,
    X grows right (0..319), Y grows down (0..239).
    """
    z = point[2] - CAMERA_Z
    if z < 0.1:
        z = 0.1
...
def face_colour(face_id: int, lum: float) -> int:
    """Map face ID + luminance to Agon 64-colour palette index.

    Each face has a distinct base colour tinted by diffuse lighting.
    Agon mode 8 palette: index = (R_2bit << 4) | (G_2bit << 2) | B_2bit
    """
    lum = max(0.0, min(1.0, lum))
    r_scale, g_scale, b_scale = FACE_COLOURS[face_id % len(FACE_COLOURS)]

    r = min(3, int(lum * r_scale * 4.0))
    g = min(3, int(lum * g_scale * 4.0))
    b = min(3, int(lum * b_scale * 4.0))
...
def agon_colour_to_rgb(col: int) -> str:
    ...
    Mode 8 palette is 2-2-2 RGB:
      index = (R_2bit << 4) | (G_2bit << 2) | B_2bit
      2-bit to 8-bit: 0->0, 1->85, 2->170, 3->255
    """
    r = (col >> 4) & 3
    g = (col >> 2) & 3
    b = col & 3
...
def generate_frame(vertices: np.ndarray,
    ...
    """Generate one frame as both VDP commands and canvas-friendly triangle data.

    Returns:
        vdp_stream: VDPStream with VDU bytes for this frame
        canvas_tris: list of {color: "#rrggbb", verts: [[x1,y1],[x2,y2],[x3,y3]]}
    """
    transformed = (rot @ vertices.T).T
    normals = compute_normals(transformed, triangles)

    # Build draw list: (avg_z, tri_index) — painter's algorithm
    draw_list = []
    for idx, (a, b, c) in enumerate(triangles):
        v0, v1, v2 = transformed[a], transformed[b], transformed[c]
        avg_z = (v0[2] + v1[2] + v2[2]) / 3.0

        # Backface culling: camera at CAMERA_Z (-4) looking toward +Z
        # Visible faces point TOWARD camera, i.e. normal.z < 0
        # Cull faces pointing away (normal.z >= 0) or edge-on (== 0)
        if normals[idx][2] >= 0:
            continue
    ...
    for avg_z, idx in draw_list:
        ...
        col = face_colour(face_ids[idx], lum)
...
<body>
<h3>Agon Cube Preview (320x240, 64-colour palette)</h3>
...
requestAnimationFrame(animate);
    ...
    with open(filename, "w") as f:
        f.write(html)
    print(f"[gen_cube] Wrote HTML preview to {filename}", file=sys.stderr)
...
def write_vdp_file(filename: str, frames: list[bytes], mode: int, fps: int) -> None:
    ...
    with open(filename, "wb") as f:
        ...
        for frame_data in frames:
            ...
            f.write(frame_data)
    print(f"[gen_cube] Wrote {len(frames)} frames to {filename}", file=sys.stderr)


def run_preview(frames_iter, total_frames: int, port: int,
    ...
    # General Poll to unlock VDP firmware
    init = VDPStream()
    init.general_poll()
    server.send_vdu(init.get_bytes())
    print("[gen_cube] Sent General Poll (VDP init handshake)", file=sys.stderr)
    ...
    server.send_vdu(init.get_bytes())
    print(f"[gen_cube] Sent mode({SCREEN_MODE}) + pixel coords + cursor off",
          file=sys.stderr)
    ...
    try:
        for frame_stream, _canvas_tris in frames_iter:
            ...
            frame_num += 1
            if frame_num % 60 == 0:
                print(f"[gen_cube] Frame {frame_num}/{total_frames} "
                      f"({len(frame_bytes)}B)", file=sys.stderr)
    except KeyboardInterrupt:
        print(f"\n[gen_cube] Stopped at frame {frame_num}", file=sys.stderr)
    finally:
        server.shutdown()
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate spinning cube VDP stream / HTML preview")
    ...
    print(f"[gen_cube] Cube: 8 vertices, 12 triangles, 6 faces",
          file=sys.stderr)
    print(f"[gen_cube] Generating {num_frames} frames...", file=sys.stderr)
    ...
    def frame_generator():
        for i in range(num_frames):
            ...
            yield generate_frame(vertices, triangles, face_ids, rot)
    ...
    if args.html:
        ...
        for i, (vdp_stream, canvas_tris) in enumerate(frame_generator()):
            ...
            if (i + 1) % 60 == 0:
                print(f"[gen_cube] Generated {i + 1}/{num_frames} frames",
                      file=sys.stderr)
        elapsed = time.time() - t0
        print(f"[gen_cube] Generation took {elapsed:.1f}s", file=sys.stderr)
