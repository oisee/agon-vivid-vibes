#!/usr/bin/env python3
"""Generate a spinning tesseract (4D hypercube) wireframe.

The tesseract has 16 vertices (all ±1 in 4D) and 32 edges (pairs
differing in exactly one coordinate). 4D rotation in XW and YZ planes,
...
# -- Camera / projection --
STEREO_DISTANCE = 3.5   # stereographic projection distance (larger = less distortion)
FOV_FACTOR = 80          # perspective 3D→2D scale
CAMERA_Z = -3.0          # camera position (closer)
...
for i in range(16):
    for j in range(i + 1, 16):
        diff = i ^ j
        if diff & (diff - 1) == 0:  # exactly one bit differs
            EDGES.append((i, j))
...
def generate_frame(frame_idx, total_frames):
    """Generate one frame of the tesseract animation."""
    t = frame_idx / total_frames
    angle_xw = t * 2 * math.pi * 0.7 * (total_frames / 60)
    angle_yz = t * 2 * math.pi * 0.5 * (total_frames / 60)
    ...
    for i, j in EDGES:
        ...
        hue = (avg_w + 2) * 90  # map -2..2 to 0..360
        lightness = 0.35 + 0.3 * (avg_w + 2) / 4  # 0.35..0.65
...
<head>
<meta charset="utf-8">
<title>Agon Tesseract Preview</title>
<style>
...
<body>
<h3>Agon Tesseract Preview (320x240, 64-colour palette)</h3>
...
requestAnimationFrame(animate);
    ...
    with open(filename, "w") as f:
        f.write(html)
    print(f"[gen_tesseract] Wrote HTML preview to {filename}", file=sys.stderr)
...
def write_vdp_file(filename, frames, mode, fps):
    ...
    with open(filename, "wb") as f:
        ...
        for frame_data in frames:
            ...
            f.write(frame_data)
    print(f"[gen_tesseract] Wrote {len(frames)} frames to {filename}", file=sys.stderr)
...
def run_preview(frames_iter, total_frames, port, verbose=False):
    ...
    print("[gen_tesseract] Sent General Poll", file=sys.stderr)
    ...
    init.cursor(False)
    server.send_vdu(init.get_bytes())
    print(f"[gen_tesseract] Sent mode({SCREEN_MODE}) + pixel coords", file=sys.stderr)
    ...
    try:
        for frame_stream, _canvas in frames_iter:
            ...
            frame_num += 1
            if frame_num % 60 == 0:
                print(f"[gen_tesseract] Frame {frame_num}/{total_frames} ({len(frame_bytes)}B)", file=sys.stderr)
    except KeyboardInterrupt:
        print(f"\n[gen_tesseract] Stopped at frame {frame_num}", file=sys.stderr)
    finally:
        server.shutdown()


def main():
    parser = argparse.ArgumentParser(description="Generate tesseract wireframe")
    ...
    print(f"[gen_tesseract] Tesseract: {len(VERTICES_4D)} vertices, {len(EDGES)} edges", file=sys.stderr)
    print(f"[gen_tesseract] Generating {num_frames} frames...", file=sys.stderr)
    ...
    if args.html:
        ...
        print(f"[gen_tesseract] Generation took {elapsed:.1f}s", file=sys.stderr)
